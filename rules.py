...
def total_borrowing(data: dict, financial_index):
    ...
    long_term_borrowing = bs.get("long_term_borrowings", 0.0)
    short_term_borrowing = bs.get("short_term_borrowings", 0.0)
    total_borrowings = float(long_term_borrowing) + float(short_term_borrowing)
...
def iscr(data: dict, financial_index):
    ...
    try:
        pnl = data["financials"][financial_index]["pnl"]["lineItems"]
        profit_before_interest_and_tax  = pnl.get("profit_before_interest_and_tax", 0.0)
